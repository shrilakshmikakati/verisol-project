...
/// @title shrilakshmik — Auto-generated from E-Contract KG
/// @notice Jurisdiction: Indian Courts | Governing Law: Indian Law
contract shrilakshmik {
    ...
    modifier onlyParty() { require(msg.sender==Professor || msg.sender==Candidate || msg.sender==Intern || msg.sender==Sangharatna_GodboleyDt || msg.sender==SHRILAKSHMI_KAKATI_shrilakshmika || msg.sender==Sangharatna_Godboley||msg.sender==owner,'Not a party'); _; }
    // ── Values extracted from e-contract ────────────────────────────
    uint256 public constant VALUE_10000 = 10000;
    uint256 public constant VALUE_2025 = 2025;
    constructor(
        uint256 _totalValue,
        uint256 _penaltyBps,
        uint256 _penaltyPeriod,
        uint256 _liabilityCap, address _Professor, address _Candidate, address _Intern, address _Sangharatna_GodboleyDt, address _SHRILAKSHMI_KAKATI_shrilakshmika, address _Sangharatna_Godboley,
	...
	assert(!(tenantObligations[id].status==TenantObligationStatus.PENDING));
	assert(!(!(tenantObligations[id].status==TenantObligationStatus.PENDING)));
	...
	assert(!(tenantObligations[id].deadline > 0));
	assert(!(!(tenantObligations[id].deadline > 0)));
